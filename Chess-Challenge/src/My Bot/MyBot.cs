...
public class MyBot : IChessBot
{
    ...
    int[] mgVal = { 0, 82, 337, 365, 477, 1025, 10000 };
    int[] egVal = { 0, 94, 281, 297, 512, 936, 10000 };
        ...
        foreach (bool stm in new[] { true, false })
        {
            for (var p = PieceType.Pawn; p <= PieceType.King; p++)
            {
                ...
                while (mask != 0)
                {
                    ...
                    mg += getPstVal(ind) + mgVal[piece];
                    eg += getPstVal(ind + 64) + egVal[piece];
                }
            }
    ...
    public int Search(Board board, Timer timer, int alpha, int beta, int depth, int ply)
    {
        ...
        for (int i = 0; i < moves.Length; i++)
        {
            if (timer.MillisecondsElapsedThisTurn >= tf)
                return 30000;
    ...
    public int GetTimeFactor(Board board, Timer timer)
    {

        if (board.PlyCount < 10)
        {
            return timer.MillisecondsRemaining / 100;
        }

        else if (board.PlyCount < 25)
        {
            return timer.MillisecondsRemaining / 20;
        }
